...
private:
	...
	std::unordered_map<int32_t, Auth::Server> server_list;
public:
	~AuthServer() 
	{
		_stopped = true;
...
public:
	...
	//协议处理
	int32_t ProtocolHandler(Auth::META_TYPE type_t, std::string stuff);
	//服务器列表处理
	void Inser(Auth::Server server) { server_list.emplace(server.server_id(), server); }
	void Erase(int32_t server_id) { server_list.erase(server_id); }
	void Erase(Auth::Server server) { server_list.erase(server.server_id()); }
};
