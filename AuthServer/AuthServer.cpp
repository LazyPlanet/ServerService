...
#define PARSE(name) \
Auth::name proto; bool result = proto.ParseFromString(stuff); \
if (!result) \
{ \
...
//协议处理
int32_t AuthServer::ProtocolHandler(Auth::META_TYPE type_t, std::string stuff)
{
	switch (type_t)
	{
		case Auth::META_TYPE_S2S_REGISTER:
		{
			PARSE(RegisterServer);
		}
		break;
