...
Player::Player()
{
	...
	AddHandler(Asset::META_TYPE_C2S_LOGOUT, std::bind(&Player::CmdLogout, this, std::placeholders::_1));
...
bool Player::HandleMessage(const Asset::MsgItem& item)
{
	...
	case Asset::MSG_TYPE_AOI_ENTER:
	{
		...
		Asset::SurroundChanged proto;
		proto.set_action_type(Asset::SurroundChanged_ACTION_TYPE_ACTION_TYPE_ENTER);
	...
	case Asset::MSG_TYPE_AOI_LEAVE:
	{
		...
		Asset::SurroundChanged proto;
		proto.set_action_type(Asset::SurroundChanged_ACTION_TYPE_ACTION_TYPE_LEAVE);
		proto.set_entity_id(common_prop.entity_id());
		this->SendProtocol(proto); 
	}
	break;
	...
	case Asset::MSG_TYPE_AOI_MOVE:
	{
		...
		Asset::SurroundChanged proto;
		proto.set_action_type(Asset::SurroundChanged_ACTION_TYPE_ACTION_TYPE_MOVE);
		proto.set_entity_id(common_prop.entity_id());
		this->SendProtocol(proto); 
	}
	break;
...
void Player::SendProtocol(pb::Message& message)
{
	...
	//进入发送队列
	this->GetSession()->EnterQueue(std::move(content));
}
